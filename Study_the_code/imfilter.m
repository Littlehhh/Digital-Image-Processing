...
if (isSeparable(a, h))
  ...
  if change_class
    ...
    if ismatrix(h) && isa(a,'single')
        b = filterSingle2DWithConv(a, h, sameSize, convMode, pad, boundary);
    else
        % Pad input based on dimensions of filter kernel.
        a = padImage(a,pad,boundary);
...
function result = filterSingle2DWithConv(a, h, sameSize, convMode, padSize, padding)
